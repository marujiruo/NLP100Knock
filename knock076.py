...
for text, filenum in zip(get_docs_list(indir_path), range(len(get_docs_list(indir_path)))):
    print "create: "+outdir_path+"data"+str(filenum)+".f"
    ...
    # 出力
    fwrite = open(outdir_path+"data"+str(filenum)+".f", "w")
    for np in np_list:
        fwrite.write("# "+" ".join([tok["w"] for tok in np])+"\n")
        ...
        fwrite.write("\t".join([article, wm1, fw, fpos, w0, fwfpos,
